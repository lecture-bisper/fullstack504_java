...
//    버블정렬 방식으로 오름차순 정렬
//    정렬 시작 시 0번 index와 다른 index를 차례대로 비교하여 0번 index가 더 클 경우 해당 index의 값과 서로 위치 변경을 하면서 마지막 index까지 진행
//    그 다음에는 1번 index를 기준으로 다른 index와 차례대로 비교하여 1번 index가 더 클 경우 해당 index의 값과 서로 위치 변경을 하면서 마지막 index까지 진행
//    이 작업을 0번 index부터 마지막 index까지 반복적으로 진행
    for (int i = 0; i < numSort.length; i++) {
...
//
//    int big = 0; // 비교 후 더 큰수를 저장하기 위한 변수2
//
